...
class CreateSchoolForm(forms.Form):
	...
	phone = forms.CharField(max_length = 15, help_text = 'Please enter in the form as, 18681112222, no spaces or dashes', label='School Phone')
...
class CreateTeacherForm(forms.Form):
	...
	password = forms.CharField(
		label=_("*Password must be at least 8 characters and should only have alphanumeric characters and/or @#$%^&+= special characters"),
	...
	def clean_password(self):	
		if "password" in self.cleaned_data and "password_confirm" in self.cleaned_data:
			...
			if re.match(r'[A-Za-z0-9@#$%^&+=]{8,}',self.cleaned_data["password"]):  
				...
				raise forms.ValidationError(_("You must enter at least 8 characters for a password. Only alphanumeric and @#$%^&+=, characters are allowed."))
