...
from .models import School, Address, Teacher, Parent_Guardian, Parent_Address, Student, Student_Address, Subject, Class, StudentAttendsClass
...
from .forms import StudentModelForm, UserModelForm, AddressModelForm, ParentModelForm, TeacherModelForm, SubjectModelForm, ClassModelForm
...
def profile_parent_address_update(request, parent_id, address_id):
	if request.method == 'POST':
		...
		print address
		addressForm = AddressModelForm(instance = address)
		return render(request, 'primary/view_student_address_update.html', {'parent': parent, 'addressForm':addressForm})
"""
"""	
@login_required
def create_parent(request):
	if request.method == "POST":
		...
		if form.is_valid():
			...
			if(first and last and gender and cell and username and password and email and home_street and home_town and home_city and home_country and 
			home_phone and work_name and work_phone):
			
				home_address = Address(street = home_street, town = home_town, city = home_city, country = home_country, phone=home_phone)
				home_address.save()
				...
				user.save()
				user_id = user
				parent = Parent_Guardian(gender = gender, personal_email = email, parent = user_id, cell_phone = cell, personal_fax = fax,
		...
		if form.is_valid():
			...
			if(student_first and student_last and student_gender and student_email and student_dob and
				...
				father_home_address = Address(
					street = father_home_street, town = father_home_town, city = father_home_city, country = father_home_country, phone= father_home_phone
					)
				...
				#Create User 
				father_user = User(username = father_username, first_name = father_first, last_name = father_last, password = father_password, email = father_email)
				father_user.save()
				...
				parent = Parent_Guardian(
					gender = father_gender, personal_email = father_email, parent = father_user_id, cell_phone = father_cell, personal_fax = father_fax,
				...
				#Create User
				user = User(
				username = student_username, first_name = student_first, last_name = student_last, password = student_password, 
				email = student_email
				)
				user.save()
				user_id = user
				...
				student = Student(gender = student_gender, email = student_email, student= user_id, dob = student_dob, medical_conditions = student_medical,
				...
				mother_home_address = Address(
					street = mother_home_street, town = mother_home_town, city = mother_home_city, country = mother_home_country, phone= mother_home_phone
					)
				...
				#Save User Model
				mother_user = User(username = mother_username, first_name = mother_first, last_name = mother_last, password = mother_password, email = mother_email)
				mother_user.save()
				...
				parent2 = Parent_Guardian(
					gender = mother_gender, personal_email = mother_email, parent = mother_user_id, cell_phone = mother_cell, personal_fax = mother_fax,
	...
	if request.method == "POST":
		...
		if form.is_valid():
			...
			if(first and last and gender and cell and username and password and email and school):
				...
				user.save()
				user_id = user
				teacher = Teacher(gender = gender, personal_email = email, school = school, teacher = user_id, cell_phone = cell, personal_fax = fax)
				teacher.save()
			else: 
				print "error"
		 # redirect to a new URL:
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			if(street and town and city and country and phone and name and principal and email and type):
				...
				address = Address(street = form.cleaned_data['street'], town = form.cleaned_data['town'], city = form.cleaned_data['city'], 
				country = form.cleaned_data['country'], phone = form.cleaned_data['phone'])
				address.save()
				address_id = address
			
				school = School (
				address = address_id, name = form.cleaned_data['name'], principal = form.cleaned_data['principal'], email = form.cleaned_data['email'],
